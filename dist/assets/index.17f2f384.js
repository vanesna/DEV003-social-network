...
const Welcome = (onNavigate2) => {
  ...
  textIntro.textContent = "Divi\xE9rtete descubriendo datos interesantes de las plantas de tu interes y descubre historias o proyectos de amantes de las plantas como t\xFA. Like Plants te ayuda a comunicarte y compartir con las personas que piensan en verde.";
...
const db = oh(app);
const crearDocumentoUsuario = (usuario, nombre, foto) => Ul(Aa(db, "users", usuario.uid), {
  id: usuario.uid,
  nombre,
  email: usuario.email,
  photoURL: foto
});
const colRef = Ta(db, "posts");
const likes = [];
const savePost = (usuario, text) => {
  Ql(colRef, {
    idu: usuario.uid,
...
const queryInstruction = sl(colRef, hl("createdAt", "desc"));
const onGetPosts = (callback) => {
  jl(queryInstruction, callback);
};
const deletePost = (id2) => Gl(Aa(db, "posts", id2));
const getPost = (id2) => Ol(Aa(db, "posts", id2));
const updatePost = (id2, newInfo) => Kl(Aa(db, "posts", id2), newInfo);
const storage = getStorage(app);
const saveFiles = (file, filename) => new Promise((resolve) => {
...
const toLike = (id2, idu) => Kl(Aa(db, "posts", id2), {
  likes: of(idu)
});
const toDislike = (id2, idu) => Kl(Aa(db, "posts", id2), {
  likes: uf(idu)
});
...
const Login = (onNavigate2) => {
  ...
  forgotPassword.textContent = "Olvid\xE9 mi contrase\xF1a";
  forgotPassword.setAttribute("class", "preguntaPassword");
  const GoogleButton = document.createElement("button");
  GoogleButton.innerHTML = '<i class="fa-brands fa-google"></i>  Inicia sesi\xF3n con Google';
  ...
  loginButton.addEventListener("click", (e) => {
    ...
    userLogin.then((userCredential) => {
      ...
      if (errorCode === "auth/wrong-password") {
        errorMessage.innerHTML = "Contrase\xF1a incorrecta";
      }
      if (errorCode === "auth/invalid-email") {
        errorMessage.innerHTML = "Ingresa un correo electr\xF3nico v\xE1lido";
      }
      if (errorCode === "auth/internal-error") {
        errorMessage.innerHTML = "Ingresa la contrase\xF1a";
      }
      if (errorCode === "auth/user-not-found") {
        errorMessage.innerHTML = "El correo electr\xF3nico ingresado no ha sido registrado";
      }
...
const RegisterComponent = (onNavigate2) => {
  ...
  inputForEmail.placeholder = "Correo electr\xF3nico";
  ...
  inputForPassword.placeholder = "Contrase\xF1a";
  ...
  password2.placeholder = "Confirma contrase\xF1a";
  ...
  textoPregunta.textContent = "\xBFYa tienes cuenta?";
  ...
  botonVolverAlogin.textContent = "Inicia Sesi\xF3n";
  formulario.append(
    inputForEmail,
    inputForPassword,
    password2,
  ...
  function validatePassword() {
    ...
    if (passwordValidate !== confirmPassword) {
      errorMessage.innerHTML = "Las contrase\xF1as no coinciden";
    ...
    if (functionPassword === true) {
      ...
      authToken.then((userCredential) => {
        ...
        if (errorCode === "auth/weak-password") {
          errorMessage.innerHTML = "La contrase\xF1a debe contener al menos 6 car\xE1cteres";
        }
        ...
        if (errorCode === "auth/internal-error") {
          errorMessage.innerHTML = "Ingresa una contrase\xF1a";
        }
        if (errorCode === "auth/email-already-in-use") {
          errorMessage.innerHTML = "El correo electr\xF3nico ingresado ya ha sido registrado";
        }
...
const Wall = (onNavigate2) => {
  ...
  iconMenu.addEventListener("click", () => {
    ...
    <li><a href='https://angiecombita.my.canva.site/dafbvb8btgu' class='option' id='option2'>Gu\xEDa de plantas</a></li>
    <li><a class='option' id='option3'>Cerrar Sesi\xF3n</a></li>
    ...
    miPerfil.addEventListener("click", () => onNavigate2("/profile"));
    const grupos = document.getElementById("option2");
    grupos.addEventListener("click", () => onNavigate2("/grupos"));
    const cerrarSesion = document.getElementById("option3");
    cerrarSesion.addEventListener("click", () => {
      logOut().then(() => {
        localStorage.removeItem("user");
        alert("Cierre de sesi\xF3n exitoso");
        onNavigate2("/");
      }).catch(() => {
        alert("Algo paso");
      });
      ...
      if (currentUser === idOwnerPost) {
        if (counterLikes > 0) {
          containerCadaPost.innerHTML += `
                  ...
                  <button class='btn-delete' id= '${doc.id}'>${"\u{1F5D1}\uFE0F"} </button>
                  <button class='class-edit' id= '${doc.id}'>${"\u{1F58D}\uFE0F"}</button>
                  </div>`;
        } else {
          containerCadaPost.innerHTML += `
                  <p class= 'class-name'>${post.nombre}</p><br>
    ...
    btnEdit.forEach((btn) => {
      btn.addEventListener("click", async ({ target }) => {
        ...
        confirmEditBtn.addEventListener("click", () => {
          ...
          id2 = target.id;
          updatePost(id2, { post: postNuevo.value });
        });
    ...
    btnLike.forEach((btn) => {
      btn.addEventListener("click", async ({ target }) => {
        ...
        if (post.likes.includes(currentUser)) {
          toDislike(id2, currentUser);
        } else {
          toLike(id2, currentUser);
        }
      });
    });
  });
...
const Profile = (onNavigate2) => {
  ...
  nombreSocialNetwork.addEventListener("click", () => onNavigate2("/wall"));
  containerHeader.append(iconMenu, nombreSocialNetwork, search, iconNotificaciones);
